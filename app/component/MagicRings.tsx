...
// Definimos el tipo correcto para los uniforms
type UniformsType = {
  uTime: THREE.IUniform<number>;
  uAttenuation: THREE.IUniform<number>;
  uResolution: THREE.IUniform<THREE.Vector2>;
  uColor: THREE.IUniform<THREE.Color>;
  uColorTwo: THREE.IUniform<THREE.Color>;
  uLineThickness: THREE.IUniform<number>;
  uBaseRadius: THREE.IUniform<number>;
  uRadiusStep: THREE.IUniform<number>;
  uScaleRate: THREE.IUniform<number>;
  uRingCount: THREE.IUniform<number>;
  uOpacity: THREE.IUniform<number>;
  uNoiseAmount: THREE.IUniform<number>;
  uRotation: THREE.IUniform<number>;
  uRingGap: THREE.IUniform<number>;
  uFadeIn: THREE.IUniform<number>;
  uFadeOut: THREE.IUniform<number>;
  uMouse: THREE.IUniform<THREE.Vector2>;
  uMouseInfluence: THREE.IUniform<number>;
  uHoverAmount: THREE.IUniform<number>;
...
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    // Crear uniforms con el tipo correcto
    // Crear uniforms como Record
const uniforms: Record<string, THREE.IUniform<any>> = {
  ...
  uBurst: { value: 0 },
    ...
    const material = new THREE.ShaderMaterial({ 
      vertexShader, 
      fragmentShader, 
      uniforms: uniforms as unknown as { [uniform: string]: THREE.IUniform<any> },
      transparent: true 
    });
  ...
  const containerStyle: CSSProperties = blur > 0 ? { filter: `blur(${blur}px)` } : {};
  
  return <div ref={mountRef} className="magic-rings-container" style={containerStyle} />;
}